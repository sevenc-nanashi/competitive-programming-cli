...
system('cargo', 'build', '--locked', '--features', 'mock', '--target-dir', 'target', exception: true)
...
Dir.mktmpdir('cpg-demo-') do |directory|
  ...
  FileUtils.mkdir_p([File.join(config, 'problem_template'), cookies, workspace, bin])
  ...
  system(
    ...
    'vhs', 'demo/demo.tape', '--output', File.join(directory, 'demo.gif'), exception: true
  )
  FileUtils.cp(%w[demo.gif demo.cast].map { |name| File.join(directory, name) }, 'docs/public')
end
